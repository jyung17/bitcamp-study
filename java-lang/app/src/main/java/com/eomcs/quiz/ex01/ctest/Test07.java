...
/*
 * n!(n 팩토리얼) 값을 조사하여 몇 개의 0으로 끝나는 지(trailing zeros) 개수를 리턴하라. 예1) 10!의 값은 두 개의 0으로 끝난다. 예2) 23!은
 * 4개의 값으로 끝난다. (23!의 값은 4바이트를 초과하기 때문에 테스트하지 말아라!)
 */
/*
 * 원문 Given an integer n, find the number of trailing zeros in the decimal representation of n! (the
...
public class Test07 {
  ...
  static int factorialTrailingZeros(int n) {
    int result = 0;
    int primeFactor = 5;
    // 5의 제곱승이 팩토리얼의 n 값 보다 작을 때만 반복한다
    while (primeFactor <= n) {
