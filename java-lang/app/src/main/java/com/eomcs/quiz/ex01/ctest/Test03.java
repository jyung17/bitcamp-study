...
public class Test03 {
  ...
  static int swapBits(int value, int i, int j) {
    int iBit = (value >>> i) & 1; // i만큼 시프트해서 & 1연산을 이용해 원하는 값을 추출
    int jBit = (value >>> j) & 1;

    System.out.println(iBit);
    System.out.println(jBit);

    //두 비트의 값이 다를 경우에만 비트 맞교환을 수행한다. -> if문 사용
    if (iBit != jBit) {
      // 두 비트의 값만 투과시킬 마스클 만든다.
      int iMask = 1 << i; // i번째 비트가 1인 마스크
      int jMask = 1 << j; // j번째 비트가 1인 마스크

      int bitMask = iMask | jMask; // i번째와 j번째가 1인 마스크
      value = value ^ bitMask; // 두 비트를 맞교환 하려는 값에 마스크를 XOR한다.
    }
