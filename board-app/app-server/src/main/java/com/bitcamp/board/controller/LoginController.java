...
public class LoginController extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      // 클라이언트에게 쿠키 보내기
      // - 쿠키 데이터는 문자열만 가능하다.(객체 저장 불가능하다)
      Cookie cookie = new Cookie("email", email); // 클라이언트 쪽에 저장할 쿠리 생성
      ...
      List<Cookie> coookies = new ArrayList<>();
      coookies.add(cookie);

      request.setAttribute("cookies", coookies);
