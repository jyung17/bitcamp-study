...
public class LinkedList {
  private Node head; // 첫 노드를 주소를 저장.
  ...
  public Object delete(int index) {
    ...
    if (head == tail) { // 마지막 남은 노드를 제거할 때
      ...
      head.value = null; // 노드의 들어 있는 값 객체의 주소를 비운다.
      head = tail = null;
      return deleted;
    }
    // 삭제할 노드를 찾기 전에 시작 노드를 head로 설정한다.
    Node cursor = head;
    ...
    // 삭제할 노드를 초기화 시킨다.
    // => 삭제 garbage 객체가 살아 있는 객체를 참조하지 않게 한다.
